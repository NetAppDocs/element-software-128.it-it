---
permalink: api/reference_element_api_setsslcertificate.html 
sidebar: sidebar 
keywords: set,setting,ssl,cert,certificate,cluster,private,key,setsslcertificate 
summary: È possibile utilizzare il metodo SetSSLCertificate per impostare un certificato SSL utente e una chiave privata per i nodi di archiviazione nel cluster. 
---
= SetSSLCertificate
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Puoi usare il `SetSSLCertificate` metodo per impostare un certificato SSL utente e una chiave privata per i nodi di archiviazione nel cluster.


NOTE: Dopo aver utilizzato l'API, è necessario riavviare il nodo di gestione.



== Parametri

Questo metodo ha i seguenti parametri di input:

|===
| Nome | Descrizione | Tipo | Valore predefinito | Necessario 


 a| 
certificato
 a| 
La versione testuale codificata PEM del certificato.  *Nota:* quando si imposta un certificato di nodo o cluster, il certificato deve includere l'estensione extendedKeyUsage per serverAuth.  Questa estensione consente di utilizzare il certificato senza errori sui sistemi operativi e sui browser più comuni.  Se l'estensione non è presente, l'API rifiuterà il certificato come non valido.
 a| 
corda
 a| 
Nessuno
 a| 
SÌ



 a| 
chiave privata
 a| 
Versione testuale codificata PEM della chiave privata.
 a| 
corda
 a| 
Nessuno
 a| 
SÌ

|===


== Valori di ritorno

Questo metodo non ha valori di ritorno.



== Richiedi esempio

Le richieste per questo metodo sono simili al seguente esempio:

[listing]
----
{
    "method" : "SetSSLCertificate",
    "params" : {
        "privateKey": "-----BEGIN RSA PRIVATE KEY-----\\nInDXvTgXMdMoet4BKnftelrXFKHgGqXJoczq4JWzGSIHNgvkrH60\n-----END RSA PRIVATE KEY-----\n",
        "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n"
    },
    "id" : 2
}
----


== Esempio di risposta

Questo metodo restituisce una risposta simile al seguente esempio:

[listing]
----
{
    "id" : 2,
    "result" : {}
}
----


== Nuovo dalla versione

10,0
