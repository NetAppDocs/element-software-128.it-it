---
permalink: api/reference_element_api_getsslcertificate.html 
sidebar: sidebar 
keywords: SSL,certificate,cert,get,getting,active,GetSSLCertificate 
summary: È possibile utilizzare il metodo GetSSLCertificate per recuperare il certificato SSL attualmente attivo sui nodi di archiviazione del cluster. 
---
= Ottieni certificato SSL
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Puoi usare il `GetSSLCertificate` metodo per recuperare il certificato SSL attualmente attivo sui nodi di archiviazione del cluster.



== Parametri

Questo metodo non ha parametri di input.



== Valori di ritorno

Questo metodo ha i seguenti valori di ritorno:

|===
| Nome | Descrizione | Tipo 


 a| 
certificato
 a| 
Il testo completo del certificato codificato in PEM.
 a| 
corda



 a| 
dettagli
 a| 
Le informazioni decodificate del certificato.
 a| 
Oggetto JSON

|===


== Richiedi esempio

Le richieste per questo metodo sono simili al seguente esempio:

[listing]
----
{
    "method" : "GetSSLCertificate",
    "params" : {},
    "id" : 1
}
----


== Esempio di risposta

Questo metodo restituisce una risposta simile al seguente esempio:

[listing]
----
{
    "id": 1,
    "result": {
        "certificate": "-----BEGIN CERTIFICATE-----\\n-----END CERTIFICATE-----\n",
        "details": {
            "issuer": "/C=US/ST=NV/L=Denver/O=NetApp/emailAddress=test@netapptest.org",
            "modulus": "",
            "notAfter": "2027-03-06T22:50:26Z",
            "notBefore": "2017-03-08T22:50:26Z",
            "serial": "CC1B221598E37FF3",
            "sha1Fingerprint": "1D:70:7A:6F:18:8A:CD:29:50:C7:95:B1:DD:5E:63:21:F4:FA:6E:21",
            "subject": "/C=US/ST=NV/L=Denver/O=NetApp/emailAddress=test@netapptest.org"
        }
    }
}
----


== Nuovo dalla versione

10,0
