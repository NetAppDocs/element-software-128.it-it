...
summary: È possibile utilizzare il metodo ListVirtualVolumeTasks per ottenere un elenco delle attività del volume virtuale nel sistema. 
---
...
[role="lead"]
Puoi usare il `ListVirtualVolumeTasks` metodo per ottenere un elenco di attività del volume virtuale nel sistema.
...
virtualVolumeTaskIDs
 a| 
Un elenco di ID attività di volume virtuale per i quali recuperare informazioni.  Se si omette questo parametro, il metodo restituisce informazioni su tutte le attività del volume virtuale.
 a| 
matrice UUID
 a| 
Nessuno
...
compiti
 a| 
Un elenco di oggetti che descrivono le attività del volume virtuale nel cluster.
 a| 
xref:reference_element_api_task_virtual_volumes.adoc[compito]vettore
