...
summary: È possibile utilizzare il metodo GetHardwareConfig per ottenere le informazioni sulla configurazione hardware di un nodo. 
---
...
[role="lead"]
Puoi usare il `GetHardwareConfig` metodo per ottenere le informazioni sulla configurazione hardware di un nodo.  Questi dati di configurazione sono destinati all'uso interno.  Per ottenere un inventario più utile dei componenti hardware del sistema live, utilizzare `GetHardwareInfo` metodo invece.
...
| configurazione hardware | Elenco delle informazioni hardware e delle impostazioni correnti. | Oggetto JSON 
|===
