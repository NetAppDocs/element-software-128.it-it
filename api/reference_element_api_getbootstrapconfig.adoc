---
permalink: api/reference_element_api_getbootstrapconfig.html 
sidebar: sidebar 
keywords: cluster,getting,bootstrap,information,node,GetBootstrapConfig 
summary: 'È possibile utilizzare il metodo GetBootstrapConfig per ottenere informazioni sul cluster e sui nodi dal file di configurazione bootstrap.  Utilizzare questo metodo API su un singolo nodo prima che venga unito a un cluster.  Le informazioni restituite da questo metodo vengono utilizzate nell"interfaccia di configurazione del cluster quando si crea un cluster.' 
---
= OttieniBootstrapConfig
:allow-uri-read: 
:icons: font
:imagesdir: ../media/


[role="lead"]
Puoi usare il `GetBootstrapConfig` metodo per ottenere informazioni su cluster e nodi dal file di configurazione bootstrap.  Utilizzare questo metodo API su un singolo nodo prima che venga unito a un cluster.  Le informazioni restituite da questo metodo vengono utilizzate nell'interfaccia di configurazione del cluster quando si crea un cluster.



== Parametri

Questo metodo non ha parametri di input.



== Valori di ritorno

Questo metodo ha i seguenti valori di ritorno:

|===
| Nome | Descrizione | Tipo 


 a| 
Nome cluster
 a| 
Nome del cluster.
 a| 
corda



 a| 
mvip
 a| 
Indirizzo MVIP del cluster.  Vuoto se il nodo non fa parte di un cluster.
 a| 
corda



 a| 
NomeNodo
 a| 
Nome del nodo.
 a| 
corda



 a| 
nodi
 a| 
Elenco di informazioni su ciascun nodo in attesa attiva di unirsi al cluster. Valori possibili:

* chassisType: (stringa) Piattaforma hardware del nodo.
* cip: (stringa) Indirizzo IP del cluster del nodo.
* compatibile: (booleano) indica se il nodo è compatibile con il nodo su cui è stata eseguita la chiamata API.
* hostname: (stringa) Nome host del nodo.
* mip: (stringa) Indirizzo IP di gestione IPv4 del nodo.
* mipV6: (stringa) Indirizzo IP di gestione IPv6 del nodo.
* nodeType: (stringa) Nome del modello del nodo.
* versione: (stringa)Versione del software attualmente installato sul nodo.

 a| 
Matrice di oggetti JSON



 a| 
svip
 a| 
Indirizzo SVIP del cluster.  Null se il nodo non fa parte di un cluster.
 a| 
corda



 a| 
versione
 a| 
Versione del software Element attualmente installata sul nodo chiamato da questo metodo API.
 a| 
corda

|===


== Richiedi esempio

Le richieste per questo metodo sono simili al seguente esempio:

[listing]
----
{
   "method": "GetBootstrapConfig",
   "params": {},
   "id" : 1
}
----


== Esempio di risposta

Questo metodo restituisce una risposta simile al seguente esempio:

[listing]
----
{
	"id":1,
	"result":{
		"clusterName":"testname",
		"nodeName":"testnode",
		"svip": "10.117.1.5",
		"mvip": "10.117.1.6",
		"nodes":[
			{
				"chassisType":"R630",
				"cip":"10.117.115.16",
				"compatible":true,
				"hostname":"NLABP1132",
				"mip":"10.117.114.16",
				"mipV6":"fd20:8b1e:b256:45a::16",
				"nodeType":"SF2405",
				"role":"Storage",
				"version":"11.0"
			},
			{
				"chassisType":"R630",
				"cip":"10.117.115.17",
				"compatible":true,
				"hostname":"NLABP1133",
				"mip":"10.117.114.17",
				"mipV6":"fd20:8b1e:b256:45a::17",
				"nodeType":"SF2405",
				"role":"Storage",
				"version":"11.0"
			},
			{
				"chassisType":"R630",
				"cip":"10.117.115.18",
				"compatible":true,
				"hostname":"NLABP1134",
				"mip":"10.117.114.18",
				"mipV6":"fd20:8b1e:b256:45a::18",
				"nodeType":"SF2405",
				"role":"Storage",
				"version":"11.0"
			}
		],
		"version":"11.0"
	}
}
----


== Nuovo dalla versione

9,6



== Trova maggiori informazioni

xref:reference_element_api_createcluster.adoc[Crea cluster]
