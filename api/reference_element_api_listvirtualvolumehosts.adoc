...
[role="lead"]
Puoi usare il `ListVirtualVolumeHosts` metodo per ottenere un elenco di tutti gli host di volumi virtuali noti al cluster.  Un host di volume virtuale è un host VMware ESX che ha avviato una sessione con il provider API VASA.
...
virtualVolumeHostIDs
 a| 
Un elenco di ID host di volumi virtuali per i quali recuperare informazioni.  Se si omette questo parametro, il metodo restituisce informazioni su tutti gli host del volume virtuale.
 a| 
...
Un elenco di oggetti che descrivono gli host dei volumi virtuali nel cluster.
 a| 
xref:reference_element_api_host.adoc[ospite]vettore
